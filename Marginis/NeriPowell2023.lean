...
Authors: Bjørn Kjos-Hanssen, Kawika O'Connor
-/
...
example (f : ℕ → ℝ) (c : ℝ) : (recursiveSeq f c) → (∀ ε > 0, ∃ N : ℕ, ∀ k : ℕ, (k ≥ N) → f k < ε) := by
  unfold recursiveSeq
  intro h
  by_contra hc
  push_neg at hc
  ...
  have seqDecreasing : (∀ n : ℕ, f (n + 1) ≤ f n) := by
    ...
    have cLeq1 : (c ≤ 1) := by
      rw [le_iff_eq_or_lt]
      tauto
    apply le_trans ((my_seq_assump (n)).2)
    have : 0 ≤ f n := (my_seq_assump n).1
    have : c * f n ≤ 1 * f n := by
        exact mul_le_mul_of_nonneg_right cLeq1 this
    simp_all
  ...
  have getContra : (∀ m : ℕ, (0 < f k_0) → (ε_0/(f k_0)) ≤ c^m) := by
    ...
    exact (div_le_iff₀ fk0StrictgtZero).mpr epsRelTofkm
